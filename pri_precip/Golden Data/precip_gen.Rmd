...
#gapData1$orificeHeater<-c(rep("000",times=((nrow(gapData)/2)-3000)),rep("100",times=500),rep("110",times=1000),rep("111",times=500),rep("000",times=((nrow(gapData)/2)+1000)))
...
#cleanData1$orificeHeater<-c(rep("000",times=((nrow(gapData)/2)-3000)),rep("100",times=500),rep("110",times=1000),rep("111",times=500),rep("000",times=((nrow(gapData)/2)+1000)))


##Add ofifice heaer info into dataset
...
inletHeatQMs <- aggregate(data[,c("inletHeat1","inletHeat2","inletHeat3")], list(time=cut(data$time,breaks="5 min")),FUN="mean",na.rm = TRUE)
...
precip.min.means <- aggregate(data[,c("Freq1","Freq2","Freq3","DEPTH1","DEPTH2","DEPTH3")], list(time=cut(data$time,breaks="min")),mean,na.rm = TRUE)
...
colnames(precipOut)<-c("time","delta1","delta2","delta3","intPrecip","lowDepthQF","exDeltaQF","missingWireInfoQF","gapQF", "priorDepthQF","overflowQF","inletHeaters1QM","inletHeaters2QM","inletHeaters3QM")
...
QM<-precipOut[,c("time","inletHeaters1QM","inletHeaters2QM","inletHeaters3QM")]
